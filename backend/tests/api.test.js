...
process.env.TAMV_SIGNING_MODE = 'ed25519';
...
test('sign + verify cycle should be valid', async () => {
  ...
  assert.equal(verification.ed25519, true);
});


test('GET /v1/audit/metrics returns federated metrics payload', async () => {
  const response = await fetch(`${base}/v1/audit/metrics`);
  assert.equal(response.status, 200);
  const data = await response.json();
