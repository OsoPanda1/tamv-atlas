...
export function buildAuditMetrics({ pidReport, signingProfile }) {
  const pidFormatScore = toPercent(
    Object.values(pidReport.formatValidation).filter(Boolean).length,
    Object.values(pidReport.formatValidation).length,
  );

  const federationBase = [
  ...
  const federations = federationBase.map((f) => ({
    ...f,
    wiring: f.id === 'FED-01' ? Math.max(f.wiring, pidFormatScore) : f.wiring,
    production: f.id === 'FED-01' && pidReport.passed ? Math.max(f.production, 75) : f.production,
  }));

  return {
    checkedAt: new Date().toISOString(),
    source: 'tamv-identity-api',
    ...
    productionAxes: [
      ...
      { axis: 'Seguridad/PQC', actual: signingProfile.mlDsaEnabled ? 72 : 58, objetivo: 95 },
    ...
    riskMatrix: [
      {
        ...
        impact: 'ALTO',
        probability: pidReport.passed ? 'BAJA' : 'MEDIA',
      ...
      },
      {
        ...
        impact: 'ALTO',
        probability: signingProfile.mlDsaEnabled ? 'BAJA' : 'MEDIA',
